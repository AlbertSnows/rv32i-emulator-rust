...
Instead of building one, we wil load an unmodified **OpenSBI** binary, the
...
- full S-mode CSR set (`sepc`, `scause`, `stval`,
...
Lots of places could use design improvements.
Test harness refactor, bit fields, maybe even making it such
that state mutations only happen in a specific way? Lots
to think about. 