...
for the handoff between them to make any sense at all. 
...
[`hw/riscv/boot.c`](https://github.com/qemu/qemu/blob/master/hw/riscv/boot.c). 
This project already matches QEMU's `virt` addresses/layout elsewhere, and
seeing the *actual*, currently-shipping implementation is a stronger
